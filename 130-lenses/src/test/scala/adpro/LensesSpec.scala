...
class LensesSpec extends FlatSpec with Checkers {
  ...
  // concrete lenses.
   ...
   def PutGet[C,A] (l: Lens[C,A]) (implicit aA: Arbitrary[A], aC: Arbitrary[C]) :Prop = forAll((a: A, c: C) => {
     l.get(l.set(a)(c)) == a
   })


  def GetPut[C,A] (l: Lens[C, A]) (implicit aC: Arbitrary[C]): Prop = forAll((c: C) => {
    l.set(l.get(c))(c) == c
  })

  def PutPut[C,A] (l: Lens[C, A]) (implicit aA: Arbitrary[A], aC: Arbitrary[C]) = forAll((a: A, c: C) => {
  ...
  def PartialPutGet[C,A] (l: Optional[C,A]) (implicit aA: Arbitrary[A], aC: Arbitrary[C]): Prop = forAll((a: A, c: C) => {
    //l.setOption(a) (c).flatMap(c => l.getOption(c).map(_a => _a == a)).getOrElse(false)
    l.getOption(l.set(a)(c)).forall(_a => _a == a)
  })

  def PartialGetPut[C,A] (l: Optional[C,A]) (implicit aC: Arbitrary[C]): Prop = forAll((c: C) => {
//    l.getOption(c).flatMap(a => l.setOption(a)(c)).forall(c0 => l.getOption(c).forall(c1 => c0 == c1))
    l.getOption(c).forall(a => l.set(a)(c) == c)
  })

  def PartialPutPut[C,A] (l: Optional[C, A]) (implicit aA: Arbitrary[A], aC: Arbitrary[C]): Prop = forAll((a: A, c: C) => {
    l.setOption(a)(c).flatMap(c2 => l.setOption(a)(c2)) == l.setOption(a)(c)
  })
