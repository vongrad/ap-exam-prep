...
class FingerTreeSpecWasowski extends FlatSpec with Checkers {
  ...
  // in your properties)
   ...
   def fingerTreeOfN[A] (n: Int, gen: Gen[A]) :Gen[FingerTree[A]] = Gen.listOfN(n, gen).map(a => Digit.toTree(a))
  ...
  // comprehensions).
   def fingerTree[A] (gen: Gen[A]) :Gen[FingerTree[A]] = Gen.choose(0, 100).flatMap(n => fingerTreeOfN(n, gen))
  ...
  it should "produce a queue containing element inserted from right" in {
    assert(Empty().addL(42).toList == List(42))
  }

  it should "produce a queue containing elements inserted from right" in check {
    forAll(Gen.listOfN(100, Gen.choose(0, 1000))) (l => {
