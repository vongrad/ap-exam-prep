...
object data {
  ...
  trait Reduce[F[_]] {
    ...
    def toList[A](fa: F[A]): List[A] = reduceR[A, List[A]]((a, b) => a :: b)(fa, List.empty[A])

    // page 6
    //
    def toTree[A](fa: F[A]): FingerTree[A] = reduceR(FingerTree.addL[A])(fa, Empty())
  }
  ...
  object NilTree {
    // we use the same extractor for both left and right views
  ...
  object Digit extends Reduce[Digit] {
    // uncomment once the interfaces are provided
  ...
  object FingerTree extends Reduce[FingerTree] {
    ...
    //        case Deep(pr, m, sf) => ConsL(pr.head, Deep(pr.tail, m, sf))
    //      }
    ...
    def deepR[A](pr: Digit[A], m: FingerTree[Node[A]], sf: Digit[A]): FingerTree[A] = sf match {
      case Nil => m match {
        ...
        case ConsR(t, h) => Deep(pr, t, Node.toList(h).reverse)
      }
      case _ => Deep(pr, m, sf)
    }
